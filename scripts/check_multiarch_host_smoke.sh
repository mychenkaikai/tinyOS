...
require_log_line '\[host-smoke\] completed arch=aarch64 profile=headless scenario=default ticks=240 events=3' "AArch64 host smoke completed run" "${LOG_AARCH64}"
...
require_log_line '\[host-smoke\] completed arch=riscv64 profile=headless scenario=default ticks=240 events=3' "RISC-V host smoke completed run" "${LOG_RISCV64}"
