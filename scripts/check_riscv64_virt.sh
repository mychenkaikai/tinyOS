...
BUILD_SCRIPT="${ROOT_DIR}/scripts/build_riscv64_virt.sh"
TMP_DIR="${ROOT_DIR}/build/tmp/check-riscv64-virt"
...
printf '== riscv64 virt check ==\n'

if ! command -v qemu-system-riscv64 >/dev/null 2>&1; then
    fail "qemu-system-riscv64 is required for the riscv64 virt check"
fi

"${BUILD_SCRIPT}"
mkdir -p "${TMP_DIR}"

timeout -k 2s 6s "${ROOT_DIR}/scripts/run_qemu_riscv64_virt.sh" > "${SERIAL_LOG_FILE}" 2>&1 || true

require_log_line 'tinyOS kernel bootstrap ready\.' "Kernel bootstrap banner reached"
require_log_line 'Architecture: riscv64' "Architecture banner reached"
require_log_line 'Platform: riscv64-virt' "Platform banner reached"
...
printf '== riscv64 virt check passed: %d checks ==\n' "${PASS_COUNT}"
