...
#### Scenario: 区分不同启动验证状态
- **WHEN** 项目对外描述当前启动支持范围
- **THEN** 必须明确区分“`QEMU + OVMF` 已验证”“其它虚拟机已验证、已准备或未验证”“真机 U 盘路径已准备但尚未验证”这三类状态，而不是混成一句“镜像能启动”
...
#### Scenario: 真机 smoke test 记录
- **WHEN** 开发者执行第一次或后续真机 `UEFI` U 盘测试
- **THEN** 仓库中必须存在可复用的 smoke test 记录模板，用于记录机器信息、固件状态、镜像提交、结果和失败阶段

#### Scenario: 写盘前镜像自检
- **WHEN** 开发者准备将当前原始镜像写入 U 盘或交给后续验证者
- **THEN** 仓库中必须存在可本地执行的镜像自检入口，用于验证 `MBR`、`ESP/FAT16`、`BOOTX64.EFI` 与 `KERNEL.BIN` 的基本布局正确性
