...
- [x] Task 13: 建立真机 `UEFI` U 盘启动路径
  - [x] SubTask 13.1: 提供将当前原始镜像写入 U 盘的最小操作说明与数据风险提示
  - [x] SubTask 13.2: 明确真机启动前提条件，例如 `x86_64 UEFI`、可从外部介质启动、以及串口/屏幕观察点
  - [x] SubTask 13.3: 设计真机 `UEFI` 启动 smoke test 记录方式，至少能记录成功进入内核或失败停留阶段

- [x] Task 14: 将启动介质验证纳入统一验收基线
  - [x] SubTask 14.1: 扩展当前验证文档，覆盖 `QEMU + OVMF`、后续真机 U 盘启动和各自的验收口径
  - [x] SubTask 14.2: 为镜像产物、启动方式、支持边界和排障入口补充复验清单
  - [x] SubTask 14.3: 保证后续每次迭代都能区分“`QEMU + OVMF` 可跑”“其它虚拟机已验证”“真机 U 盘可跑”这三类状态，而不是混成一句“能启动”
